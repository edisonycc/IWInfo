...
public class JwtTokenUtils {
    ...
    @Value("${jwt.header}") //defined in the application.yml
...
//                .signWith(SignatureAlgorithm.HS512, tokenSecret).compact();
        ...
        return Jwts.builder().setClaims(claims)
                .setExpiration(expirationDate)
                .signWith(generateKeyByDecoders()).compact();
    }
    ...
    private Claims getClaimsFromToken(String token) {
        ...
        try {
            ...
            claims = Jwts.parserBuilder().setSigningKey(tokenSecret).build().parseClaimsJws(token).getBody();
        } catch (Exception e) {
            claims = null;
        }
